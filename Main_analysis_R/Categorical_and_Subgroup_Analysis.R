...
get.df.analysis <- function(df){
  ...
  df$Admit_Year <- relevel(df$Discharge_Year, ref='2013-2016')
...
# Create several categories for each component to estimate odds ratios
...
get.Pulse.results <- function(df){
  ...
  for (threshold in thresholds){
    i <- which(thresholds == threshold)
    if (i==1){
      results <- get.OR(df.Death.analysis, 'Pulsepressure_below_120', 30, threshold, 'Death')
      m[1, 1:2] <- results[[1]][1:2]
      m[i+1, 1:2] <- results[[1]][3:4]
      m[i+1, 3:5] <- results[[2]]
    } else {
  ...
  df.results <- as.data.frame(m, row.names = c('30 <= PP < 40mmHg', 'PP >=50mmHg', 'PP >=40mmHg', 'PP <30mmHg', 'PP <20mmHg'))
  colnames(df.results) <- c('n', 'Death (%)', 'Adjusted_OR', '95% CI', 'p-value')
  return(df.results)
}
...
# Investigate heterogeneity of the effects of hypotension on ICU mortality
df.subgroup <- df.Death.analysis %>% 
  mutate(age_cat = case_when(age<45 ~ '<45',
         ...
         septic_shock = if_else(((Lac=='2<=Lac<5' | Lac=='>=5') & NEE_mcg_perkg_perminx1000>0), 1, 0),
...
# Estimate adjusted OR in each subgroup
get.subgroup.OR <- function(df, outcome, subgroup, potential.covariates){
  ...
  } else if (subgroup == 'septic_shock') {
    covariates <- potential.covariates[-which(potential.covariates=='Lac')]
    levs <- c(0, 1)
...
results.subgroup.septic_shock <- get.subgroup.OR(df.subgroup, 'Death', 'septic_shock', potential.covariates)
...
subresults.table <- rbind(results.subgroup.age, results.subgroup.vasopressor, results.subgroup.septic_shock,
                          results.subgroup.vent, results.subgroup.cancer, results.subgroup.admtype, results.subgroup.overall)
