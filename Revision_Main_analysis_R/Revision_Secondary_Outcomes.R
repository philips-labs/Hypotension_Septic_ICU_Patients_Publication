...
df.AKI <- read.csv('Sepsis_AKI_composite_revision.csv')
sum(df.AKI$AKI)
mean(df.AKI$AKI)

df.MI <- read.csv('Sepsis_MI_composite_revision.csv')
sum(df.MI$MI)
mean(df.MI$MI)

binary_variables_composite <- c('male', 'aspirin', 'diuretics', 'ace_inhibitors', 'ARBs', 'beta_blockers',
...
covariates <- c(binary_variables_composite, 'ethnicity', 'Admission_Type', 'Admit_Year', 'Hospital_Bed_Size', 'Hb',
...
get.predict.df <- function(df, component, outcome, percentile.range, n_bin, n_ma){
  ...
  covariates.factor <- c(binary_variables_composite, 'ethnicity', 'Admission_Type', 'Admit_Year', 'Hospital_Bed_Size', 'Hb',
...
tiff('./Figures/Supplementary/Additional_file_2.tiff', units="cm", width=18, height=12, res=300)
all.spline.hist.AKI <- get.four.plots.hist(df.AKI.analysis, 'AKI_composite', 'AKI or Death', 0.001)
all.spline.hist.AKI
dev.off()
ggsave("./Figures/Supplementary/Sepsis_spline_hist_AKI.png", all.spline.hist.AKI, height = 12, width = 18, units='cm', dpi=300)

# Myocardial Injury

tiff('./Figures/Supplementary/Additional_file_3.tiff', units="cm", width=18, height=12, res=300)
all.spline.hist.MI <- get.four.plots.hist(df.MI.analysis, 'MI_composite', 'Myocardial Injury or Death', 0.001)
all.spline.hist.MI
dev.off()
ggsave("./Figures/Supplementary/Sepsis_spline_hist_MI.png", all.spline.hist.MI, height = 12 , width = 18, units='cm', dpi=300)
